...
--end,
  ...
  test = function()
    ...
    test:call("NTOWFv1", result, "\xa4\xf4\x9c\x40\x65\x10\xbd\xca\xb6\x82\x4e\xe7\xc3\x0f\xd8\x52")

    -- [MS-NLMP] 4.2.2.1.3
    local status, result = i:SessionBaseKey()
    test:call("SessionBaseKey", result, "\xd8\x72\x62\xb0\xcd\xe4\xb1\xcb\x74\x99\xbe\xcc\xcd\xf1\x07\x84")
    test:call("KeyExchangeKey", result, "\xd8\x72\x62\xb0\xcd\xe4\xb1\xcb\x74\x99\xbe\xcc\xcd\xf1\x07\x84")

    -- [MS-NLMP] 4.2.2.2
    local status, lm_response, ntlm_response = i:ComputeResponse()
    -- [MS-NLMP] 4.2.2.2.1
    test:call("NtChallengeResponse", ntlm_response, "\x67\xc4\x30\x11\xf3\x02\x98\xa2\xad\x35\xec\xe6\x4f\x16\x33\x1c\x44\xbd\xbe\xd9\x27\x84\x1f\x94")
    test:call("LmChallengeResponse", lm_response,   "\x98\xde\xf7\xb8\x7f\x88\xaa\x5d\xaf\xe2\xdf\x77\x96\x88\xa1\x72\xde\xf1\x1c\x7d\x5c\xcd\xef\x13")

    -- [MS-NLMP] 4.2.2.2.2
    local status, kxkey = i:KXKEY()
    test:call("KXKEY", lm_response,   "\x98\xde\xf7\xb8\x7f\x88\xaa\x5d\xaf\xe2\xdf\x77\x96\x88\xa1\x72\xde\xf1\x1c\x7d\x5c\xcd\xef\x13")
    i.flags = bit.bor(i.flags, NTLMSSP_NEGOTIATE_LM_KEY)
    local status, sealkey = i:KXKEY()
    test:call("KXKEY (LM_KEY)", sealkey, "\xb0\x9e\x37\x9f\x7f\xbe\xcb\x1e\xaf\x0a\xfd\xcb\x03\x83\xc8\xa0")
    ...
    test:call("RandomSessionKey encrypted w/ KXKEY [1]", encrypted_session_key, "\x51\x88\x22\xb1\xb3\xf3\x50\xc8\x95\x86\x82\xec\xbb\x3e\x3c\xb7")

    i.flags = bit.bor(i.flags, NTLMSSP_NEGOTIATE_NON_NT_SESSION_KEY)
    local result, kxkey = i:KXKEY()
    local rc4_handle = openssl.rc4(kxkey)
    local encrypted_session_key = rc4_handle(i.random_session_key)
    test:call("RandomSessionKey encrypted w/ KXKEY [2]", encrypted_session_key, "\x74\x52\xca\x55\xc2\x25\xa1\xca\x04\xb4\x8f\xae\x32\xcf\x56\xfc")
    ...
    local rc4_handle = openssl.rc4(kxkey)
    local encrypted_session_key = rc4_handle(i.random_session_key)
    test:call("RandomSessionKey encrypted w/ KXKEY [3]", encrypted_session_key, "\x4c\xd7\xbb\x57\xd6\x97\xef\x9b\x54\x9f\x02\xb8\xf9\xb3\x78\x64")
    i.flags = bit.band(i.flags, bit.bnot(NTLMSSP_NEGOTIATE_LM_KEY))
